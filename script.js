...
const taskDetailPopover = document.getElementById("task-detail-popover");
...
function openTaskDetail(taskId) {
  ...
  if (!task || !taskDetailPopover) return;
  ...
  const start = new Date(task.startTime);
  const end = new Date(task.endTime);
  detailTime.textContent = `From ${start.toLocaleString()} to ${end.toLocaleString()}`;

  taskDetailPopover.classList.add("show");
}

function closeTaskDetail() {
  if (!taskDetailPopover) return;
  taskDetailPopover.classList.remove("show");
  detailTaskId = null;
}

if (taskDetailClose) {
  taskDetailClose.addEventListener("click", (e) => {
    e.stopPropagation();
    closeTaskDetail();
  });
}
...
if (taskDetailPopover) {
  taskDetailPopover.addEventListener("click", (e) => {
    if (e.target === taskDetailPopover) {
      closeTaskDetail();
    }
  });
}
